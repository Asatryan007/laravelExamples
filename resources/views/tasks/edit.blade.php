    ...
    <div class="container mx-auto px-5">
        ...
        <form action="{{ route('tasks.update', $task) }}" method="POST" class="max-w-lg mx-auto p-6 bg-gray-500 rounded-lg">
            ...
            @else
                <div class="mb-4">
                    <label for="title" class="block text-black">Title</label>
                    <input type="text" value="{{$task->title}}" name="title"  maxlength="255" class="w-full p-2 border border-white bg-gray-300 text-black cursor-not-allowed" disabled>
                    <input type="hidden" value="{{$task->title}}" name="title"  maxlength="255" class="w-full p-2 border border-white bg-gray-300 text-black cursor-not-allowed" >
                    @error('title')
                    <div class="alert alert-danger text-red-500">{{ $message }}</div>
                    @enderror
                </div>
                <div class="mb-8">
                    <label for="description" class="block text-black">Description</label>
                    <textarea name="description" cols="30" rows="10" class="w-full p-2 border border-white bg-gray-300 text-black cursor-not-allowed" disabled>{{$task->description}}</textarea>
                    <input type="hidden" value="{{$task->description}}" name="description"  maxlength="255" class="w-full p-2 border border-white bg-gray-300 text-black cursor-not-allowed" >
                    @error('description')
                    <div class="alert alert-danger text-red-500">{{ $message }}</div>
                    @enderror
                </div>
